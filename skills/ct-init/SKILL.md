---
name: ct-init
description: "저장소 초기화(init) 직후 `.0_my/core_project.md`, `.0_my/core_code_style.md`, `.0_my/core_workflow.md` 3개 문서를 SKILL.md 내 포맷으로 항상 재생성하고, AGENTS.md는 core 문서 참조만 유지하도록 정리한다."
---
...
## 실행 방식 (스크립트 미사용)
- 이 스킬은 **스크립트를 실행하지 않는다**.
- Codex가 아래 Markdown 포맷에 따라 파일을 직접 작성한다.
...
## 유지보수
- 외부 템플릿 파일 없이 이 `SKILL.md`의 "생성 포맷" 섹션만 수정한다.
- 스크립트는 사용하지 않으며, 재생성은 본 문서 기준으로 수행한다.
