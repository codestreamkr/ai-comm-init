...
- 초안: `초안`, `설계`, `템플릿`, `준비`, `제안`만 요청하면 대화에 구조와 내용을 제시한다.
- 반영: `생성`, `만들어줘`, `추가`, `저장`, `수정`, `반영`, `적용`이 있으면 허용된 경로에 파일을 생성하거나 수정한다.
  - 예: `새 스킬 생성`, `스킬 만들어줘`, `skills/foo에 추가`, `이 경로에 저장`, `기존 SKILL.md 수정`, `검토안을 파일에 반영`
- 초안 표현과 반영 표현이 함께 있으면 반영 표현을 우선한다.
...
1. 시스템 `skill-creator/SKILL.md`를 처음부터 끝까지 읽는다.
2. `agents/openai.yaml`을 만들거나 수정할 때 `skill-creator/references/openai_yaml.md`를 읽는다.
...
- 2026-07-13: 기존 자연어 호출 호환과 시스템 skill-creator 역할 분리, 요청 표현별 초안·파일 반영 권한, 표준 resource 구조, 동적 컴포넌트 표기와 허용 목록, Git ref 대비 전체 계약·UI 메타데이터 변경 검사, 참조·Git 추적·agents 구조 검증 순서를 정리했다.
