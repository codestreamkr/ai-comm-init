...
- legacy 적합성 게이트를 통과한 `legacy-main-test`의 MainTest에만 적용한다.
...
- 프로젝트 JUnit 버전에 맞는 메서드명 순서 annotation으로
...
{MainTestClass}
└─ testNN_*()
   ├─ reqJson fixture 준비(JUnit lifecycle setup)
   ├─ UnitTest 인스턴스 생성
   ├─ setUp() 호출
   ├─ _Test(reqJson) 호출
   └─ 실제 negative·예외 케이스(reqJson) 전체 호출
```
...
- YYYY-MM-DD: MainTest 보조 문서 작성 또는 갱신
````

고정 규칙:

1. 메인 테스트 현재 반영 범위가 partial이면 그 사실을 문서에도 표시
2. 각 `testNN_*`는 `nodeId + callPath + callNode -> 하위 테스트 메서드 -> 검증 의도`까지 적는다
...
- 2026-07-13: legacy 적합성 게이트, `nodeId + callPath + callNode` 추적, 실제 negative·예외 전체 경로, JUnit 버전에 독립적인 MainTest 순서 설명과 문서 이력을 반영했다.
