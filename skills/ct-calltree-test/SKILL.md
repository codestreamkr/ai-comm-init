...
description: "Generate Java unit tests from call-tree markdown documents. Creates tests for [TC:✅] call nodes based on actual source code analysis."
---
...
4. `target-call`이 없으면 `[TC:✅]` 전체를 대상으로 한다. 처리 순서와 진행 방식은 §기본 워크플로우를 따른다.
...
| 케이스 | 패턴 | 예시 |
|--------|------|------|
| 정상 호출 | `{methodName}_Test` | `checkGsPrdStck_Test` |
| 외부 조건 미충족 | `{methodName}_{사유}_NoCall` | `checkKCP_PaymentTypeMismatch_NoCall` |
...
구체적 사유가 있으면 `NotApplicable_NoCall`보다 구체 이름을 선호한다. 상세는 `references/calltree-test-patterns.md` §2 참조.
...
- 상세는 `references/calltree-test-patterns.md` §4 참조.
...
- helper/fixture 구조 상세는 `templates/request-fixture-template.md` 참조.
...
- 상세는 `references/calltree-test-patterns.md` §3 참조.
...
8. **ArgumentCaptor 판단**: mapping/조립 계열이면 캡처 기반 검증, 단순 위임이면 매처 사용이 적절한가
9. **fixture 사용 확인**: shared fixture가 존재하는 흐름이면 base-request.json에서 파생되었는가 (§Shared Fixture 규칙)
...
4. **노드 완료 게이트 확인** — §노드 완료 게이트의 전 항목 점검
5. **완료 보고 후 즉시 다음 노드로 이동한다.** 사용자가 개입하면 그때 멈춘다.

### Phase 2: 마무리 (모든 노드 완료 후)

1. MainTest를 `mainTestGroup` 기준으로 `testNN_*` 묶음으로 생성/갱신한다.
   - **기존 MainTest 갱신 우선 원칙**: Phase 0에서 확정한 기존 MainTest 파일이 있으면 반드시 해당 파일을 갱신한다. 계약 `mainTestClass`와 이름이 달라도 신규 파일을 만들지 않는다.
   - 기존 MainTest의 기존 `testNN_*` 순번/구조를 유지하고, 새 노드는 기존 번호 뒤에 추가한다.
...
사용자가 명시적으로 `--parallel`을 지정했을 때만 서브에이전트를 사용한다.

- 서브에이전트 1개 = 노드 1개. 같은 UnitTest 파일에 들어갈 노드는 1개 에이전트가 처리한다.
- shared fixture와 MainTest는 메인 에이전트만 생성/수정한다.
- 각 서브에이전트에게 담당 노드, UnitTest 경로, fixture 경로, 스킬 규칙을 전달한다.
- 서브에이전트는 `*UnitTest.java`, 메서드명 목록, assertion-근거 매핑, 게이트 자체 점검 결과를 반환한다.
- 메인 에이전트는 게이트 통과 여부와 누락 노드를 검수한다. 미통과 시 수정 또는 재배포.
...
2. `[TC:✅]` 노드 1개당 `testNN_*` 1개가 기본. 각 `testNN_*`는 해당 call의 `_Test` + negative 쌍을 함께 실행한다.
3. partial 상태면 클래스 Javadoc에 현재 반영 범위를 남긴다.
4. 보조 문서를 `.0_my/call-trees/{MainTestClass}_YYYYMMDD_HHMMSS.md`에 생성/갱신한다.
5. 코드 뼈대와 문서 뼈대는 `templates/` 참조.
...
### 출력 파일
- 대상 흐름의 `*UnitTest.java`
- 해당 흐름의 `*MainTest.java`
- `.0_my/call-trees/{MainTestClass}_YYYYMMDD_HHMMSS.md`
- 필요 시 `src/test/resources/{domain}/{entrypoint}/base-request.json`
...
6. CallTree와 실제 코드가 충돌할 때 스킬이 독자적으로 대상을 제외하거나 추가하는 방식
