...
**이 레퍼런스의 모든 코드 블록은 최신 기준선(Spring Security 6 / jjwt 0.12.x) 문법이다.** 감지된 환경이 다르면 그대로 복붙하면 컴파일 안 된다.
...
이 레퍼런스는 **Spring Security 분야의 구현 문법·구성 패턴**만 담당한다. 보안 아키텍처 판단과 정책 결정은 spring 에이전트가 맡는다.
