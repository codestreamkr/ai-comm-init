...
### 적용 절차 — 이 레퍼런스의 코드를 읽는 방법

**이 레퍼런스의 모든 코드 블록은 최신 기준선(Spring Boot 3 / Hibernate 6 / Jakarta) 문법이다.** 감지된 환경이 다르면 그대로 복붙하면 컴파일 안 된다.
...
이 레퍼런스는 **JPA 분야의 구현 문법·패턴**만 담당한다. 설계 판단과 아키텍처 결정은 spring 에이전트가 맡는다.
...
판단·설계 질문이 들어오면 에이전트가 먼저 정리한 뒤, 구현 단계에서 이 레퍼런스를 참고한다.
...
이 레퍼런스의 중심은 쿼리 패턴이지만, Entity 작성 시 최소한의 판단 기준은 여기서 제공한다.
...
연관관계 상세 설계, 상속 전략, Embeddable, 낙관적/비관적 락은 이 레퍼런스 범위 밖이다. 필요하면 별도 요청을 안내한다.
