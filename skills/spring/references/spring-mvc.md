...
이 레퍼런스는 **Spring MVC 분야의 구현 문법·패턴**만 담당한다. 서비스 경계, 데이터 접근 선택, 아키텍처 판단은 `spring` 에이전트가 맡는다.
...
**다루지 않는 것 (spring 에이전트 또는 다른 레퍼런스 영역)**
...
- N+1, fetch join, Projection 이슈가 보이면 JPA 레퍼런스로 이어감
